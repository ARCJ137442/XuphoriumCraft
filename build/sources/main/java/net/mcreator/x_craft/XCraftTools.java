...
public class XCraftTools extends Elementsx_craft.ModElement
{
	...
	@GameRegistry.ObjectHolder("x_craft:x_item")
	public static final Item X_ITEM=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_food")
	public static final Item X_FOOD=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_magnet")
	public static final Item X_MAGNET=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_sword")
	public static final Item X_SWORD=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_axe")
	public static final Item X_AXE=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_pickaxe")
	public static final Item X_PICKAXE=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_shovel")
	public static final Item X_SHOVEL=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_hoe")
	public static final Item X_HOE=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_shield")
	public static final Item X_SHIELD=null;
	
	@GameRegistry.ObjectHolder("x_craft:x_shield_powered")
	public static final Item X_SHIELD_POWERED=null;
	
	public XCraftTools(Elementsx_craft instance)
	{
		super(instance,2);
	}
	...
	public void registerModels(ModelRegistryEvent event)
	{
		for(int i=0;i<4;i++)
		{
			ModelLoader.setCustomModelResourceLocation(X_ITEM,i,new ModelResourceLocation("x_craft:x_item_"+i,"inventory"));
			ModelLoader.setCustomModelResourceLocation(X_SWORD,i,new ModelResourceLocation("x_craft:x_sword_"+i,"inventory"));
		}
		ModelLoader.setCustomModelResourceLocation(X_FOOD,0,new ModelResourceLocation("x_craft:x_food","inventory"));
		
		ModelLoader.setCustomModelResourceLocation(X_MAGNET,0,new ModelResourceLocation("x_craft:x_magnet","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_MAGNET,1,new ModelResourceLocation("x_craft:x_magnet_powered","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_MAGNET,2,new ModelResourceLocation("x_craft:x_magnet_powered_extra","inventory"));
		
		ModelLoader.setCustomModelResourceLocation(X_AXE,0,new ModelResourceLocation("x_craft:x_axe","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_PICKAXE,0,new ModelResourceLocation("x_craft:x_pickaxe","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_SHOVEL,0,new ModelResourceLocation("x_craft:x_shovel","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_HOE,0,new ModelResourceLocation("x_craft:x_hoe","inventory"));
		
		ModelLoader.setCustomModelResourceLocation(X_SHIELD,0,new ModelResourceLocation("x_craft:x_shield","inventory"));
		ModelLoader.setCustomModelResourceLocation(X_SHIELD_POWERED,0,new ModelResourceLocation("x_craft:x_shield_powered","inventory"));
	}
	...
	public static class XMagnet extends XCraftToolCommon
	{
		...
		public static double[] findRot(double dx,double dy,double dz,double value)
		{
			double[] result=new double[]{0,0,0};
			double distance=Math.sqrt(dx*dx+dy*dy+dz*dz);
			if(distance<=0)return result;
			/*
			double Hangle=Math.atan2(dx/distance,dz/distance);
			double Vangle=Math.cos(dy/distance);
			result[0]=value*(Math.sin(Vangle/180*Math.PI)*Math.cos((Hangle-180)/180*Math.PI))/2;
			result[1]=value*Math.sin((Hangle-180)/180*Math.PI)/2;
			result[2]=value*(-1*Math.cos(Vangle/180*Math.PI)*Math.cos((Hangle-180)/180*Math.PI))/2;
			*/
			result[0]=dx/distance*value;
			result[1]=dy/distance*value;
			result[2]=dz/distance*value;
			/*
				   ...
				   ((EntityLivingBase)entity).getHeldItemOffhand().equals(itemstack))
				{
					...
					for(Entity item : entities)
					{
						if(item instanceof EntityItem||item instanceof EntityXPOrb||level>1&&item instanceof IProjectile)
						{
							...
							//double Hangle=Math.atan2(dx/distance,dz/distance);
							//double Vangle=Math.cos(dy/distance);
							//Vec3d rot=Vec3d.fromPitchYaw((float)Hangle,(float)Vangle);//findRot(dx,dy,dz,1/(distance+1));
							item.motionX+=sgn(dx)/(2+distance);//rot.x
							item.motionY+=sgn(dy)/(2+distance);//rot.y
							item.motionZ+=sgn(dz)/(2+distance);//rot.z
						}
					}
				}
			}
