...
public class XuphoriumCraftElements implements IFuelHandler, IWorldGenerator {
	...
	public void preInit(FMLPreInitializationEvent event) {
		...
		this.addNetworkMessage(XuphoriumCraftVariables.WorldSavedDataSyncMessageHandler.class, XuphoriumCraftVariables.WorldSavedDataSyncMessage.class,
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = XuphoriumCraftVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = XuphoriumCraftVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				XuphoriumCraft.PACKET_HANDLER.sendTo(new XuphoriumCraftVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				XuphoriumCraft.PACKET_HANDLER.sendTo(new XuphoriumCraftVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			XuphoriumCraft.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
