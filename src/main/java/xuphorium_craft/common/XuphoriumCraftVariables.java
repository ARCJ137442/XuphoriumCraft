...
public class XuphoriumCraftVariables
{
	public static class MapVariables extends WorldSavedData
	{
		public static final String DATA_NAME = "xuphorium_craft_mapvars";
		...
		public void syncData(World world)
		{
			this.markDirty();
			if (world.isRemote) {
				xuphorium_craft.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(0, this));
			} else {
				xuphorium_craft.PACKET_HANDLER.sendToAll(new WorldSavedDataSyncMessage(0, this));
			}
		}
	...
	{
		public static final String DATA_NAME = "xuphorium_craft_worldvars";
		...
		public void syncData(World world)
		{
			...
			if (world.isRemote)
			{
				xuphorium_craft.PACKET_HANDLER.sendToServer(new WorldSavedDataSyncMessage(1, this));
			} else {
				xuphorium_craft.PACKET_HANDLER.sendToDimension(new WorldSavedDataSyncMessage(1, this), world.provider.getDimension());
			}
		}
	...
	public static class WorldSavedDataSyncMessageHandler implements IMessageHandler<WorldSavedDataSyncMessage, IMessage>
	{
		...
		private void syncData(WorldSavedDataSyncMessage message, MessageContext context, World world)
		{
			if (context.side == Side.SERVER) {
				if (message.type == 0)
					xuphorium_craft.PACKET_HANDLER.sendToAll(message);
				else
					xuphorium_craft.PACKET_HANDLER.sendToDimension(message, world.provider.getDimension());
			}
			if (message.type == 0) {
